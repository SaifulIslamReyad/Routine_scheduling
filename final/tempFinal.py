...
title = tk.Label(
    root,
    text="Teacher Course & Preference Entry",
    font=HEADER_FONT,
    bg="#eef5fb",
    fg="#003366",
)
...
tk.Label(teacher_tab, text="Teacher Name:", font=LABEL_FONT, bg="#f4f9ff").grid(
    row=0, column=0, padx=10, pady=10, sticky="e"
)
teacher_name_entry = tk.Entry(teacher_tab, width=30, font=DEFAULT_FONT)
teacher_name_entry.grid(row=0, column=1, padx=5)

tk.Label(teacher_tab, text="Rank:", font=LABEL_FONT, bg="#f4f9ff").grid(
...
tk.Button(
    ...
    pady=6,
    font=DEFAULT_FONT,
).grid(row=1, columnspan=4, pady=15)
...
def show_course_list():
    ...
    for c in courses:
        course_listbox.insert(
            tk.END, f"{c['code']} -  ({c['credit']} cr) [{c['teacher']}]"
        )
...
tk.Label(course_tab, text="Teacher Name:", font=LABEL_FONT, bg="#f4f9ff").grid(
    row=0, column=0, padx=10, pady=10
)
course_teacher_dropdown = ttk.Combobox(
    course_tab,
    textvariable=course_teacher_var,
    state="readonly",
    width=27,
    font=DEFAULT_FONT,
)
...
tk.Label(course_tab, text="Course Title:", font=LABEL_FONT, bg="#f4f9ff").grid(
    row=1, column=0, padx=10, pady=10
)
course_title_entry = tk.Entry(course_tab, width=20, font=DEFAULT_FONT)
course_title_entry.grid(row=1, column=1)

tk.Label(course_tab, text="Course Code:", font=LABEL_FONT, bg="#f4f9ff").grid(
...
tk.Label(course_tab, text="Credit:", font=LABEL_FONT, bg="#f4f9ff").grid(
    row=2, column=0, padx=10, pady=10
)
course_credit_entry = tk.Entry(course_tab, width=20, font=DEFAULT_FONT)
course_credit_entry.grid(row=2, column=1)

tk.Label(course_tab, text="Year:", font=LABEL_FONT, bg="#f4f9ff").grid(
...
tk.Button(
    ...
    pady=6,
    font=DEFAULT_FONT,
).grid(row=3, columnspan=4, pady=15)
...
tk.Label(pref_tab, text="Select Teacher:", bg="#f4f9ff").grid(
    row=0, column=0, padx=10, pady=10
)
pref_teacher_dropdown = ttk.Combobox(
    pref_tab, textvariable=pref_teacher_var, state="readonly", width=25
)
pref_teacher_dropdown.grid(row=0, column=1, padx=5)

slot_frame = tk.LabelFrame(
    pref_tab, text="Preferred Slots", bg="#e8f0fe", font=("Helvetica", 10)
)
slot_frame.grid(row=1, column=0, columnspan=4, padx=10, pady=10)

tk.Label(slot_frame, text="", bg="#e8f0fe").grid(row=0, column=0)
for j, slot in enumerate(SLOTS):
    tk.Label(slot_frame, text=slot_timings[slot], bg="#e8f0fe").grid(
        row=0, column=j + 1
    )
...
tk.Button(
    ...
    padx=20,
    pady=5,
).grid(row=2, columnspan=4, pady=10)
...
with open("courses.json", "r") as f:
    courses = json.load(f)
with open("teacher_rank.json", "r") as f:
    teacher_rank = json.load(f)
with open("teacher_preferences.json", "r") as f:
    teacher_preferences = json.load(f)
days = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday"]
routine = {
    year: {day: {slot: None for slot in range(1, 8)} for day in days}
    for year in [1, 2, 3, 4]
}
...
def can_assign(teacher, year, day, slot):
    return (
        routine[year][day][slot] is None and slot not in teacher_schedule[teacher][day]
    )
...
def reassign_slot(year, day, slot, requesting_teacher):
    ...
    for new_day, new_slot in prefs:
        ...
        if can_assign(current_teacher, year, new_day, new_slot):
            _move_assignment(
                year, day, slot, new_day, new_slot, course_code, current_teacher
            )
            return True
...
def _move_assignment(year, old_day, old_slot, new_day, new_slot, course_code, teacher):
    if int(course_code[-1]) % 2 == 1 and new_slot <5 and old_slot <5 :
    ...
    elif new_slot>=5 and old_slot>=5:
        routine[year][old_day][old_slot] = None
...
def printing():
    ...
    # Updated colors: 2nd & 3rd year swapped; 4th year more red
    year_colors = {1: "DEEAF6", 2: "FFE499", 3: "C5DFD3", 4: "FC8779"}
    ...
    # Example day names (update if different)
    days = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday"]
    short_days = {
        "Sunday": "SUN",
    "Monday": "MON",
    ...
    for day in days:
        ...
        for year in [1, 2, 3, 4]:
            # Short form year
            suffix = {1: "st", 2: "nd", 3: "rd"}.get(year, "th")
            year_name = f"{year}{suffix}"
            year_cell = ws.cell(row=current_row, column=2, value=year_name)
            year_cell.fill = PatternFill(
                start_color=year_colors[year],
                end_color=year_colors[year],
                fill_type="solid",
            )
            ...
            # Set row height to increase cell height
            ws.row_dimensions[current_row].height = 30
            current_row += 1

        ws.merge_cells(start_row=start_row, start_column=1, end_row=current_row - 1, end_column=1)
        day_cell = ws.cell(row=start_row, column=1, value=short_days.get(day, day))
    ...
    for row in ws.iter_rows(min_row=2, max_row=ws.max_row):
        ...
        while col <= 10:
            ...
            while (
                col + 1 <= 10
                and row[col].value == value
                and value not in ["-", "BREAK"]
            ):
                col += 1
            if col > start_col:
                ws.merge_cells(
    ...
    for row in ws.iter_rows():
        for cell in row:
            cell.font = Font(bold=True, size=12 if cell.column == 1 else 11)
