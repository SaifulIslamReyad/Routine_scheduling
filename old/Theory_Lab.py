...
name_entry.grid(row=0, column=2, columnspan=6, pady=5, sticky="we")

selected_cells = set()
buttons = {}

# Slot timings header row
tk.Label(root, text="").grid(row=1, column=0)  # Empty top-left cell
for j, slot in enumerate(SLOTS):
    time_label = slot_timings.get(slot, "")
    tk.Label(root, text=time_label, font=("Arial", 9, "bold")).grid(row=1, column=j + 1, padx=2, pady=2)

# Day labels and slot buttons
for i, day in enumerate(DAYS):
    tk.Label(root, text=day).grid(row=i + 2, column=0, padx=5)
    for j, slot in enumerate(SLOTS):
        btn = tk.Button(root, text=str(slot), width=6,
                        command=lambda d=day, s=slot: toggle_cell(d, s, buttons[(d, s)]))
        btn.grid(row=i + 2, column=j + 1, padx=2, pady=2)
...
# Save button
tk.Button(root, text="Save to JSON", command=save_to_json).grid(row=len(DAYS) + 2, column=0, columnspan=8, pady=10)
